...
- **Pupils are shown surname first**, everywhere a name is rendered: `{lastName} {firstName}`, and the seat tile shows the surname alone. In a French school a pupil is called by their surname, and the roster sorts by it, so leading with the given name reads wrong and scans wrong. Three surfaces were missed and later corrected (the seating plan's pool and pupil card, and the pupil page); if you add a fourth, match it.
