...
That standing rule — bump the version, never write an upgrade callback — had only ever been exercised by bumps that *added* a table. Phase 7 changed a table's primary key (`seats` went from `[layoutId+row+col]` to `id`, with the room stored as free positions instead of grid cells), and "bump the version" alone does not cover that case: Dexie refuses a primary-key change outright, throwing `UpgradeError: Not yet support for changing primary key` while opening the database. `init.ts` has no catch for it, so every teacher with an existing workspace would have hit a blank screen on the next load — not wiped, **bricked**, with their pupils' data still sitting in IndexedDB and no route to the wipe in Réglages to recover. The sanctioned way to change a primary key is to drop the store to `null` in its own version first (`db.version(7).stores({ seats: null })`), then declare the new shape in the version after (`db.version(8).stores({ seats: "id, layoutId, studentId, &[layoutId+x+y]" })`) — two versions, not one, for a key change. A regression test in `src/db/index.test.ts` builds a real v2→v6 database with `fake-indexeddb` and opens it with the current code, so this cannot silently regress. The next time a table's primary key needs to change, this is the pattern — a bump that only adds columns to an existing store's key is fine as a single version; a bump that changes what the key *is* needs the drop-then-recreate pair.
