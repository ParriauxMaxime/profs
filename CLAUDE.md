...
The backup format is **12**. A version-10 file is still refused whole rather than imported: its journal lived in a store that no longer exists, so half-importing it would silently drop every entry rather than refuse the file that held them. A version-11 file is different — it predates a séance's required `startsAt`/`endsAt`, but nothing in it is lost, so `parseBackup` runs it through `repairSeanceCollisions`, the same collection-level repair the v16 Dexie upgrade uses, and imports it as a v12 workspace with every séance timed and no two séances of one class on one day sharing a start. One repair function, two callers, so the rule stays what it was for v10: refuse whole only when the file holds a loss no backfill can undo.
...
**`db.version(16)` carries the first `upgrade()` callback in the codebase, and that is a deliberate exception rather than a reversal.** A séance's `startsAt` became required, with a new `endsAt` alongside it — the field-level equivalent of the changed-primary-key case above, and the drop-and-redeclare move fails it the same way. `attendance` and `behaviourEvents` are both keyed to `sessions.id`; dropping `sessions` destroys every séance and leaves a term of attendance and behaviour as rows nothing reads, nothing counts, and every export still carries — the invisible-orphan failure `cascade.ts` exists to prevent, produced this time by the rule meant to keep the schema simple. So `version(16)` keeps the store and repairs every row instead, through `repairSeanceCollisions` (`src/domain/seance.ts`): a missing `startsAt` is floored from `createdAt` — the hour a séance was created in is the hour it was taught in, since all four things that create one happen during the lesson — and a missing `endsAt` follows it by `DEFAULT_SEANCE_MINUTES`. That per-row rule alone is not enough: two untimed séances of one class on one day created in the same hour would floor to the identical `startsAt`, and `resolveSlot` — which matches by `startsAt` and always returns the first — would strand the second forever, present only in the export. `repairSeanceCollisions` repairs the whole day's group at once so the earliest-created row keeps its hour and any later collision is nudged a minute forward instead, which both the Dexie upgrade and `parseBackup` call rather than the per-row `backfillSeanceTimes` directly. The rule this bends is not abandoned: it still stands for every change that ADDS a table or a field. What it does not cover is a field becoming required underneath rows that carry dependents.

Several regression tests in `src/db/index.test.ts` build a real database at an earlier schema with `fake-indexeddb` and open it with current code — the v2 per-class layout, the v9 saved room, the v13 day-keyed journal entry, a v15 séance with no `startsAt` alongside an attendance row and a behaviour event keyed to it (asserting that both still resolve to it after the upgrade), and two v15 séances of one class on one day created within the same hour (asserting they still get different, reachable starts). **This seam is the blind spot: nothing else in the suite runs new code against an old row.** Every schema change from here wants such a test, one per store whose shape moved.
