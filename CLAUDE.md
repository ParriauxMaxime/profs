...
**`src/db/`** — Dexie. `openWorkspaceDb(workspaceId)` opens `profs-<id>`; each workspace is its own database. Nineteen tables across seven `db.version(...).stores({...})` calls: `version(2)` declares the original seven (`classes`, `students`, `subjects`, `gradebooks`, `periods`, `columns`, `grades`) together with five added in phase 2A for the classroom features — `sessions` (one row per lesson), `attendance` (keyed `[sessionId+studentId]`), `behaviourEvents` (append-only observations, `classId` denormalised for a one-index class timeline), `seatingLayouts` (one room per class), and `seats`; `version(3)` adds three more for phase 2B's rubrics: `rubricTemplates` (a reusable named list of criteria), `rubricAssessments` (one graded instance, owning a copy of its criteria — see below), and `rubricScores` (one 1–4 level per criterion per pupil, keyed `[assessmentId+criterionId+studentId]`); `version(4)` adds phase 3's two group tables — `studentGroups` (a named subset of a class) and `groupMembers` (keyed `[groupId+studentId]`), a way of selecting and viewing pupils for filtering and seating, never a thing that holds a grade; `version(5)` adds phase 4a's `scheduleEntries` — the recurring weekly timetable; `version(6)` adds phase 4b's `diaryEntries` (keyed `[classId+date]`), the journal; `version(7)` and `version(8)` together turn phase 7's room from a grid into free positions — `version(7)` drops the old `seats` store and `version(8)` recreates it as `"id, layoutId, studentId, &[layoutId+x+y]"`, the `&` marking the compound index unique so the database itself guarantees two tables never share a point, and a bug in `canPlace` surfaces as a rejected write rather than as a pupil nobody can tap — see "Schema changes are disposable" below for why the change took two versions instead of one. `provider.tsx` exposes `useDb()`; `init.ts` runs once before first render; `seed.ts` creates the demo school; `backup.ts` does JSON export/import; `cascade.ts` owns every multi-table delete (see below).
...
A `Seat` row now encodes **two** states, not three: a row with `studentId: null` is an **empty table**, and a row with a `studentId` is an **occupied** one. There is no third "gap" state any more, and that is a simplification, not a loss — the old gap (no row at all for a `[layoutId, row, col]`) existed only because a rectangular grid forces every cell to exist, so an aisle had to be a cell with nothing rendered in it. Phase 7 replaced the grid with tables at free positions: a table has its own `x`/`y`, so an aisle is simply the absence of a table, which is the absence of a row. Removing a table (`removeTable`) *is* carving the aisle; there is nothing left to carve separately.

The seating plan's gesture is pick-up-then-place, not drag: the teacher first holds a pupil or a table — from the rail, from an already-seated pupil, or from a table in layout-edit mode — and then taps a target. `Held` carries three kinds (`pool`, `seat`, `table`), and `resolveDrop`/`resolveFloorDrop` in `src/domain/room.ts` are the two pure rules for what a tap means, so the grammar lives in tested functions rather than a click handler: a pupil held from the rail always *seats* (displacing whoever already occupies the target, who returns to the rail), a pupil held from a table always *swaps* with the target (which degrades to a plain move when the target is empty), and a table held in layout-edit mode moves to wherever bare floor is tapped next. A bare tap on a seated pupil still opens their pupil card — that stays the gesture of the lesson itself, attendance and behaviour — so picking that pupil up instead goes through the card's `Déplacer` button. In layout-edit mode a bare tap on a table now picks up the TABLE, not the pupil sitting at it — moving the pupil still goes through `Déplacer`. Two further layout-edit controls exist because they are two different gestures: `×` removes the table entirely (changing the room), `↩` frees the place but leaves the table standing (changing only the arrangement).

Two spacing constants govern placement, and the second is bigger than the first for a reason worth carrying forward: `PITCH` (table plus one unit of air) steps anything rectilinear, but `canPlace`'s overlap test is per-axis, not Euclidean, and on a diagonal `max(|dx|,|dy|)` is only `distance / sqrt(2)` before rounding to integers costs up to a further unit per axis — so a curved row needs `ARC_SPACING`, a wider step, or seats on an arc collide where a straight row would have cleared.

`swapSeats` no longer takes an `expectedStudentId`. Phase 5's guard existed because a held seat was coordinates only, and another tab could move the occupant away and reseat someone else at that position before the drop landed; a table now has an id, and the id itself is the guard — reading a removed or reassigned table by id simply fails, and the write never happens.

A room starts from one of four templates (`src/domain/room-templates.ts`: `rows`, `arc`, `islands`, `u`, built by `buildRoom`) — but **a template stamps and then ceases to exist**. Nothing stored anywhere records that a room "is an arc". The alternative — a live template with stored parameters — was rejected because it cannot answer what happens when the teacher moves one table out of the arc and then changes the curvature: does that table follow, or stay behind? Both answers are wrong half the time, and expressing the difference would need an override set on top of the template. A stamp has one state, and the state is the room.

Applying a template is destructive to the TABLES and deliberately not to the ARRANGEMENT. `reseat` (`src/domain/room.ts`) pours the currently seated pupils back into the new positions in reading order, so a grid restamped as an arc keeps its front row in front. Whoever no longer fits comes back as `overflow` for the caller to warn about, before the write — never silently dropped. That is what makes a destructive stamp survivable for a teacher who spent a term arranging 28 pupils: they lose the exact chairs, not the arrangement.

Every template parameter is a count of TABLES, never of pupils, and `TEMPLATE_LIMITS` plus `clampTemplate` live in the domain rather than in the form — the same reason the subject palette and the period names were moved out of a component. A seat total plus a row count is not a shape until something decides how they split, and each generator would otherwise have had to invent that rule for itself.
...
- **State bound to a record must be anchored to that record's identity, never to its position.** This codebase has produced the same bug three times, in three disguises, and every instance risked writing to or deleting the wrong student:
  ...
  - The seating plan's `held` value is anchored to a pupil id (from the rail) or a table's own id (from a seat, or the table itself in layout-edit mode), never a rail index or a position — the rail reorders on every placement, so an index-held selection would retarget onto whoever slid into that slot. This was the last place in the codebase anchored to a position instead of a record's identity: phase 5's `swapSeats` needed an `expectedStudentId` guard purely because a held seat was `[row, col]` coordinates with no identity of its own; phase 7 gave every table an id, and the guard is now the id.
...
The Dexie schema is declared as a chain of `db.version(...).stores({...})` bumps, none of them with an upgrade callback. Phase 2A's five new tables did not get a migration from phase 1 data: there is nothing to migrate, since attendance and behaviour didn't exist before, and any stray v1 test data is treated as garbage the "supprimer toutes les données" wipe in Réglages already handles. `backup.ts` follows the same posture — a v1 backup file is rejected outright by `WorkspaceBackup`'s schema check, not upgraded, since importing it half-populated (gradebooks but no sessions) would be worse than refusing it. The rule for the next schema change: add a table or a field, bump the version, do not write an upgrade function — a stale workspace gets wiped, not migrated.

That standing rule — bump the version, never write an upgrade callback — had only ever been exercised by bumps that *added* a table. Phase 7 changed a table's primary key (`seats` went from `[layoutId+row+col]` to `id`, with the room stored as free positions instead of grid cells), and "bump the version" alone does not cover that case: Dexie refuses a primary-key change outright, throwing `UpgradeError: Not yet support for changing primary key` while opening the database. `init.ts` has no catch for it, so every teacher with an existing workspace would have hit a blank screen on the next load — not wiped, **bricked**, with their pupils' data still sitting in IndexedDB and no route to the wipe in Réglages to recover. The sanctioned way to change a primary key is to drop the store to `null` in its own version first (`db.version(7).stores({ seats: null, seatingLayouts: null })`), then declare the new shape in the version after (`db.version(8).stores({ seats: "id, layoutId, studentId, &[layoutId+x+y]", seatingLayouts: "id, classId" })`) — two versions, not one, for a key change.

`seatingLayouts` is dropped alongside `seats`, and the reason generalises past this one table: **a store whose SHAPE changed must be dropped even when its key did not**. `SeatingLayout`'s primary key is still `id`, so Dexie would have carried a v6 room forward untouched — carrying `rows`/`cols` into code that reads `width`/`height`. That row is not a crash; it is worse. `layout.width * UNIT_PX` is `NaN`, so the room renders at `transform: scale(NaN)`, `floorSlots` produces nothing because `x + TABLE <= undefined` is false, and `addTable` refuses every placement — a screen that is simply broken on first open after upgrade, and a backup taken in that window exports the zombie row intact, since the backup's zod schema is `{id}.loose()`. A layout whose every seat was just discarded carries no information anyway, which is the same disposable-not-migrated doctrine the seats got.

Two regression tests in `src/db/index.test.ts` build a real v2→v6 database with `fake-indexeddb` and open it with the current code — one asserting the seats are gone and the class survives, one asserting no `rows`/`cols` layout is carried forward. **This seam is the branch's blind spot: nothing else in the suite ever runs new code against an old row.** Every schema change from here on wants such a test, one per store whose shape moved. The next time a table's primary key needs to change, this is the pattern — a bump that only adds columns to an existing store's key is fine as a single version; a bump that changes what the key *is* needs the drop-then-recreate pair.
...
- The seating plan still has no drag and drop — deliberately: it is the one gesture the browser automation that verifies these screens cannot drive, and a keyboard equivalent (the arrow-key nudge) is needed regardless of automation, so pick-up-then-place stays the gesture. A room cannot yet be saved as a named layout reusable by another class ("Ma salle 204") — each class still gets its own `SeatingLayout`, stamped from a template and hand-tuned from there.
...
- `docs/superpowers/specs/2026-09-03-profs-room-layouts-design.md` — the room-as-room design: templates that stamp and cease to exist, half-tile geometry, and the schema's primary-key change
