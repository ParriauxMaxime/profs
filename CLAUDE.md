...
***Tous présents* is the register's one bulk gesture, and it replaced *Commencer
une séance*.** The old button made a séance and stopped — and a séance with no
attendance and no behaviour is not a lesson taught, by the rule two paragraphs
below, so its whole effect was a row that counted for nothing. This does the
thing a teacher actually starts a lesson by doing, and the séance falls out of
it: `ensureSeance`, then the marks.

It marks the REMAINING pupils, never all of them — `markRemainingPresent` in
`src/db/attendance.ts` reads and writes in one transaction and skips anyone who
already has a row, whatever it says. The gesture is "the two absences are
marked, everyone else was here", and overwriting would un-record the marks just
made; it is also the one control that touches a whole class at once, so getting
it wrong is thirty wrong facts rather than one. The count is on the label
(*Tous présents (23)*) and zero hides the button, since a control that can do
nothing is worse than none.

Writing présent explicitly is not the default `attendance.ts` refuses to
define. A default would claim a pupil was here because nobody said otherwise;
this is a teacher stating it about a class they are standing in front of.

**What went with the old button:** it had a second job — when the slot already
had a séance, it started an EXTRA one at the current clock hour, which is how an
unscheduled lesson on an already-taught day got a row. That path is gone, and
`canStart`'s "at most one séance an hour" guard with it. A free day is
unaffected (any mark creates a séance at the current hour); what cannot be done
from this page any more is a second, unscheduled séance on a day that already
has one.

**Opening the class page writes no `sessions` row.** The tab it replaced called `getOrCreateTodaySession` from an effect on mount, so glancing at a seating plan filed a lesson nobody taught — the schedule's own ruling, undone by the one screen that read it. A séance is written by the first thing actually recorded: an attendance mark, a behaviour event, note text, or *Tous présents*. Each awaits `ensureSeance`, and none of them is an effect. Be precise about the scope of that claim: it is about `sessions` and nothing else. `getOrCreatePlan` still writes a `seatingPlans` row from an effect the first time a class is looked at in a salle, which records an arrangement, not a lesson.
...
**Materialising a session per scheduled lesson stays rejected**, for the reason it always was: every holiday, strike, cancellation and sick day would leave an empty session in a pupil's timeline, filling attendance history with lessons that never occurred. Nothing is created from the timetable, and creation is still lazy; only the set of things that trigger it grew — a mark, a behaviour event, note text, *Tous présents*, each through `getOrCreateSessionAt`, which reads and writes inside one transaction so StrictMode's double-invoked effects cannot produce two rows for one lesson. The consequence is that Aujourd'hui and the class page's séance strip each merge two lists — scheduled-but-unrecorded and recorded-but-unscheduled — and a lesson that is both must render **once**. `slotsForDay` in `src/domain/seance.ts` is that merge. Every séance now carries a start, so the merge is a single pairing pass: a séance pairs with a lesson of its OWN CLASS whose start matches, and then by time — the class is not redundant, since two classes at the same minute is legal here and Aujourd'hui reads every class at once, so time alone would let one class's séance claim another's lesson. There is no second branch for a séance with no time, because that case no longer exists.

`Session.startsAt` and `Session.endsAt` — both minutes from midnight, never `"10:05"` — are what tell two lessons in one day apart; `[classId+date]` alone cannot. Both are required: a séance used to allow an absent `startsAt`, meaning "recorded before séances carried a time, or opened outside the timetable", and every reader had to cover that absence, with no answer at all once the front door needed to give a lesson a position on an hour grid. Neither field is a foreign key into the timetable: at creation both are COPIED from the schedule entry's `startMinute`/`endMinute` when the slot has one, or from the clock — floored to the hour, `+ DEFAULT_SEANCE_MINUTES` (55) — when it doesn't. Moving a lesson to another hour next term leaves every past séance holding the hour it actually happened at, and the URL's fallback absorbs the mismatch for links. `slotsForDay` draws a paired slot's OWN times rather than the entry's, for the same reason: a lesson that ran long must draw as long as it ran, and the strip lets a teacher correct one after the fact. No index is added for either field — a class has a handful of séances on a day, so `[classId+date]` fetches them and the time is matched in memory.
...
A/B week parity is **derived** from a term-start date, never stored, so no calendar of weeks can drift. The anchor lives in `localStorage` (`src/domain/term.ts`, key `profs-term-start`) rather than a table: it describes this device's workspace, has no relations, and Aujourd'hui must know the week before the database opens.
...
`TimeGrid` (`src/modules/design-system/components/time-grid.tsx`) draws that geometry and is no longer `/schedule`'s alone: `/`, the dated week described below, draws the same hours around real dates. It moved out of `src/modules/schedule/` and its interface generalised from a weekday to an opaque `GridColumn` — `{ key, label, today? }` — so the caller composes the heading (`t('schedule.day.1')` for one screen, `lun. 7` for the other) rather than the grid guessing which of the two it is drawing. A `GridLesson`'s `column` matches a column's `key`; whether the grid draws a `<Link>` or a `<button>` for a lesson is decided by whether that lesson carries an `href` — `/schedule` leaves it unset, because selecting a lesson opens a form beside the grid rather than navigating, and `/` sets it, because a block there is a real destination and a `<tr>`-style click handler takes no focus and doesn't fire on Enter, the same reason `DataTable` puts a real `<Link>` in its first cell. `gridWindow`, `layoutDay` and the `--hour` arithmetic below are untouched by any of this — the window is still computed from the lessons passed in, never from the columns drawn, so switching a phone from the week to a single day does not slide the grid.
...
### `/` is the dated week, and it is a signpost

`/schedule` draws the recurring **intention** — weekday columns, A/B badges, no dates. `/` (`src/modules/today/page.tsx`) draws the week that is actually **happening**: real dates around today, séances already recorded showing their dot, a now-line, and every block a link into that lesson. The two share `TimeGrid` and look alike on purpose; folding them into one screen was rejected, because editing a recurring entry ("every Monday of week A") through a surface that shows one dated Monday would need the alternating abstraction rebuilt from the dated grid every time a teacher wanted to change it.

**`?date` names a DAY, never a week**, local midnight in epoch ms, and the viewport decides how much of it is drawn: `≥lg` shows that day's whole week, `<lg` shows the day alone. One parameter serves both because a day belongs to exactly one week, so the narrow branch is the same screen showing less of itself rather than a second URL — a link sent from a phone opens on the right week on a tablet. A bare `/` means today; `Aujourd'hui` clears the parameter rather than writing today's date into it, which keeps the common URL clean and makes the button's meaning exact. An unparseable or absent `date` — including an empty `?date=`, which `Number` would otherwise read as `0` and silently open on 1 January 1970 — falls back to today, the same resolve-or-ignore rule `?classe` follows for a deleted class.

**Every step is `Router.replace`, never `push`.** Walking to next week is a change of view, not a navigation: a `push` per tap would make Back crawl backwards one week at a time, and a teacher who walked four weeks forward and wanted to leave would press Back four times to go nowhere.
...
`monthGrid` in `src/domain/calendar.ts` is the calendar's `weekParity`: a grid wrong by one day still looks exactly like a calendar, and nobody checks a calendar against another calendar. Its test walks every month of two years. Nothing here adds days by arithmetic — `nextDay` walks the calendar, because `+ 86_400_000` is wrong at each DST change and eventually a whole day out. `addDays(ms, n)`, alongside it, is `nextDay`/`previousDay` repeated `n` times rather than a shortcut around them — the same rule `rangeStart` (`src/domain/behaviour-range.ts`) already applied on its own. It absorbed the journal's own week-shift loop, which stepped the same way by hand, and `/` uses it for the same reason: a step that added `n × 86_400_000` would drift at a DST change exactly as `weekParity` warns.
...
**A rename sidesteps the two-version dance entirely, rather than needing it done in one version instead of two.** The rubric rekey — `rubricScores`, keyed against a `RubricAssessment` that no longer exists, becoming `criterionLevels`, keyed `[columnId+criterionId+studentId]` — changed a primary key and did it in a single declaration. Dexie diffs a version against whatever it already has BY NAME: a name it has never held has no prior key to conflict with, so a rename is not "change this store's key," which throws, but "drop one store, create an unrelated one," which does not. The dance above is still what a store keeping its OWN name needs; it was never what this rekey needed, because nothing in the new declaration was still called `rubricScores`.
...
`UpgradeError` (what a primary-key change throws), for `VersionError` (what a
stale service-worker shell meets if it ever reaches a database bumped past
it — the downgrade trap above means this schema cannot produce one, but a
later one still could), and for an unknown name.
...
Every multi-table delete lives in `src/db/cascade.ts`, each a single `rw` transaction covering every table it touches. `deleteBehaviourEvent` is single-table but lives there too, so every delete is in one place. `setColumnCriteria` in `src/db/criterion-levels.ts` is the same pattern at a finer grain: replacing a rubric column's criteria drops the `criterionLevels` of any criterion that didn't survive, in one transaction. If you find yourself writing a multi-table delete inline in a component, stop and add it there with tests instead — an orphaned grade row is invisible in the UI, never averaged, and survives export/import.
...
### The shell is found, shared and installed — still without a request

Everything a teacher sees before the app has loaded lives in `public/`, and it is the one corner where the no-network promise is easiest to break by accident. A share card, a favicon and a font all *want* to come from somewhere else. **Every one of them is a file in this repository**, referenced by an absolute URL on the Pages origin. An `og:image` pointed at a card generator, a `<link>` to a CDN, or a JSON-LD block that loads anything are each a request made on a reader's behalf in the one place nobody audits — and `README.md` and `PRIVACY.md` say there are none.
...
**`screenshots` in the manifest are what make Chrome show a rich install dialog** rather than a bare prompt, and they are real captures of the running app. Chrome wants the long side within 2.3× the short one and one aspect ratio per `form_factor`.

The invitation to install is `InstallInvitation`, on **Aujourd'hui only**, at the bottom. `installOffer` in `src/domain/install.ts` decides: standalone or dismissed outrank everything, a held `beforeinstallprompt` beats iOS instructions, and anything else — desktop Firefox, macOS Safari, a Chromium that has not fired the event — renders **nothing**, because an invitation with no way to accept it only asks. iOS Safari gets a sentence, since it has no such event and never will. The dismissal is permanent on the device and there is deliberately no second entry in Réglages: every browser that could have shown the button already carries install in its own menu. It is an ordinary element, never a dialog — the ban reaches here too.
...
- **A `rubric` column is skipped by the pupil page's carnet sections**, and that
  is this merge's doing rather than either branch's. The section dispatches on
  `column.type` precisely so a new type lands as one more case — but the case a
  grille needs is a per-critère surface, and `EditableCell` returns `null` for
  the type, so rendering one would draw a row carrying a label, a coefficient
  and nothing at all. A dead row reads as a bug; an omission at least matches
  what the page can honestly show. The fix is a rubric row that opens the
  pupil's levels read-only, and it wants the same design pass the grid's
  `RubricCellButton` got — not a hole plugged during a merge.
